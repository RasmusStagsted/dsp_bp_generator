...
class BeltRouter:
    ...
    def __init__(self, pos, input_count, output_count, product_count, selector_belt_indicies, belt_length):
        ...
        self.generate_output_and_product_belts(pos, input_count, output_count, product_count, belt_length)
    ...
    def generate_splitters(self, pos, input_count, output_count, product_count):
        ...
        for i in range(product_count):
            ...
            product_splitter = Buildings.Splitter(temp_pos, Yaw.North)
    ...
    def generate_input_belts(self, pos, input_count, belt_length):
        ...
        for i in range(input_count):
            ...
            belts = Buildings.Belt.generate_belt(temp_pos, Yaw.South, belt_length)
            belt_start = belts[0]
            belt_end = belts[-1]
    ...
    def generate_output_and_product_belts(self, pos, input_count, output_count, product_count, belt_length):
        self.output_belts = []
        for i in range(output_count):
            temp_pos = Pos(pos.x + 2 * (i + input_count), pos.y, 1)
            belts = Buildings.Belt.generate_belt(temp_pos, Yaw.North, belt_length)
            belt_start = belts[0]
            belt_end = belts[-1]
            ## Connect belts to splitters
    ...
    def generate_selector_belts(self, pos, input_count, output_count, product_count, selector_belt_indicies):
        ...
        for i in range(len(selector_belt_indicies)):
            ...
            belts = Buildings.Belt.generate_belt(temp_pos, Yaw.North, 1 + len(selector_belt_indicies) - i)
            ...
            belts = Buildings.Belt.generate_belt(temp_pos, Yaw.East, (input_count + output_count + product_count - selector_belt_indicies[i]) * 2)
    ...
    def generate_production_belts(self, pos, input_count, output_count, product_count):
        ...
        for i in range(product_count):
            ...
            belts = Buildings.Belt.generate_belt(temp_pos, Yaw.West, 2 + 2 * i)
            ...
            belts = Buildings.Belt.generate_belt(temp_pos, Yaw.North, i + 2)
            north_belt_start = belts[0]
            north_belt_end = belts[-1]
            # Connect cornor
            # TODO remove function "connect_cornor_belt" and implement it in Belt.connect_to_belt
